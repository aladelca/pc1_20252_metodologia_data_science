...
def make_features(
	series: pd.Series, lags=(1, 2, 3, 5, 7, 14, 21, 28), mas=(7, 14, 28)
) -> pd.DataFrame:
	df = pd.DataFrame({'target': series})
	for L_ in lags:
		df[f'lag_{L_}'] = series.shift(L_)
	for w_ in mas:
		df[f'ma_{w_}'] = series.shift(1).rolling(w_).mean()
	idx = df.index
...
def _infer_lags_from_columns(cols: List[str]) -> List[int]:
	lags = []
	for c_ in cols:
		if c_.startswith('lag_'):
			try:
				lags.append(int(c_.split('_')[1]))
			except Exception:
				pass
...
def _infer_mas_from_columns(cols: List[str]) -> List[int]:
	mas = []
	for c_ in cols:
		if c_.startswith('ma_'):
			try:
				mas.append(int(c_.split('_')[1]))
			except Exception:
				pass
...
def recursive_forecast_xgb(
	series: pd.Series, model, feature_columns: List[str], horizon: int
) -> pd.DataFrame:
	...
	for _ in range(horizon):
		next_date = (
			(series_ext.index.max() + pd.Timedelta(days=1))
			if len(series_ext)
...
def main():
	...
	parser.add_argument(
		'--data',
		required=True,
		help='Parquet con transaction_date, product_brand, events',
	)
