...
def make_features(series, lags=(1, 2, 3, 5, 7, 14, 21, 28), mas=(7, 14, 28)):
	...
	for La in lags:
		df[f'lag_{La}'] = series.shift(La)
	for Ma in mas:
		df[f'ma_{Ma}'] = series.shift(1).rolling(Ma).mean()
	idx = df.index
...
def main():
	...
	default_models_dir = (
		Path(__file__).resolve().parent.parent.parent / 'src' / 'models'
	).resolve()
	parser.add_argument(
		'--model_name',
	...
	xgb_metrics = train_xgboost(series, test_days=args.test_days, model_pkl_path=model_pkl_path)
