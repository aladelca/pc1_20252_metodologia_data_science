...
class CleaningConfig:
	...
	# Marca
	brand_candidates: List[str] = field(
		default_factory=lambda: [
			'product_brand',
			'productBrand',
			'brand',
	...
	brand_placeholders: List[str] = field(
		default_factory=lambda: [
			'(not set)',
			'',
			' ',
	...
	non_negative_keywords: List[str] = field(
		default_factory=lambda: [
			'revenue',
			'transaction',
			'pageviews',
...
def _winsorize(s_: pd.Series, low: float, high: float) -> pd.Series:
	if s_.dropna().empty:
		return s_
	ql, qh = s_.quantile([low, high])
	return s_.clip(lower=ql, upper=qh)
...
def normalize_strings_and_placeholders(df: pd.DataFrame, cfg: CleaningConfig) -> pd.DataFrame:
	...
	for c_ in obj_cols:
		...
		if cfg.strip_whitespace:
			out[c_] = out[c_].apply(lambda x: x.strip() if isinstance(x, str) else x)
		if cfg.to_lower_categoricals:
			out[c_] = out[c_].apply(lambda x: x.lower() if isinstance(x, str) else x)

		# placeholders -> NaN (pero NO tocar "(not set)" en la marca)
		if c_ == brand_col or c_ == 'product_brand':
			out[c_] = out[c_].apply(
				lambda x: np.nan if isinstance(x, str) and x in placeholders_without_notset else x
			)
		else:
			out[c_] = out[c_].apply(
				lambda x: np.nan if isinstance(x, str) and x in placeholders_all else x
			)
...
def drop_rows_invalid_date(
	df: pd.DataFrame, cfg: CleaningConfig, dqr: DataQualityReport
) -> pd.DataFrame:
...
def drop_high_missing_columns(
	...
	targets = [
		c_
		for c_ in missing_pct[missing_pct > cfg.drop_columns_missing_threshold].index.tolist()
		if c_ not in protected
	]
	if not targets:
		return df
	...
	for c_ in targets:
		...
		if pd.api.types.is_numeric_dtype(ser):
			...
			if pd.isna(mean_val):
				mean_val = 0
			out[c_] = ser.fillna(mean_val)
			imputed_cols.append(c_)
		else:
...
def coerce_types(
	...
	for c_ in out.columns:
		...
		if out[c_].dtype == 'object':
			coerced = pd.to_numeric(out[c_], errors='coerce')
...
def negatives_to_nan(df: pd.DataFrame, cfg: CleaningConfig, dqr: DataQualityReport) -> pd.DataFrame:
	...
	for c_ in num_cols:
		if any(k_ in c_.lower() for k_ in cfg.non_negative_keywords):
			negs = (out[c_] < 0).sum()
			if negs > 0:
				out.loc[out[c_] < 0, c_] = np.nan
				affected.append(c_)
...
def impute_missing(df: pd.DataFrame, cfg: CleaningConfig) -> pd.DataFrame:
	out = df.copy()
	# numéricas
	for c_ in out.select_dtypes(include=[np.number]).columns:
		if out[c_].isna().any():
			if cfg.impute_numeric == 'median':
				out[c_] = out[c_].fillna(out[c_].median())
			elif cfg.impute_numeric == 'mean':
				out[c_] = out[c_].fillna(out[c_].mean())
			elif cfg.impute_numeric == 'zero':
				out[c_] = out[c_].fillna(0)
	# categóricas / bool
	for c_ in out.select_dtypes(include=['object', 'category', 'bool']).columns:
		if out[c_].isna().any():
			if cfg.impute_categorical == 'mode':
				mode = out[c_].mode(dropna=True)
				fillv = mode.iloc[0] if not mode.empty else cfg.unknown_token
				out[c_] = out[c_].fillna(fillv)
			elif cfg.impute_categorical == 'unknown':
				out[c_] = out[c_].fillna(cfg.unknown_token)
	return out
...
def handle_outliers(df: pd.DataFrame, cfg: CleaningConfig, dqr: DataQualityReport) -> pd.DataFrame:
	...
	if not cfg.winsorize:
		return out
	for c_ in out.select_dtypes(include=[np.number]).columns:
		out[c_] = _winsorize(out[c_], *cfg.winsor_limits)
		dqr.winsorized_cols.append(c_)
	return out
...
def drop_constant_cols(
	...
	const_cols = [c_ for c_ in nunique[nunique <= 1].index.tolist() if c_ not in protected]
	if const_cols:
...
def reduce_categoricals_cardinality(
	...
	cat_cols = [
		c_
		for c_ in out.select_dtypes(include=['object', 'category']).columns
		if c_ not in protected
	]
	for c_ in cat_cols:
		n_unique = out[c_].nunique(dropna=True)
		if 0 < n_unique <= cfg.rare_apply_on:
			vc = out[c_].value_counts(dropna=True)
			rare = vc[vc < cfg.rare_thresh].index
			if len(rare) > 0:
				out[c_] = out[c_].where(~out[c_].isin(rare), other='Otros')
	return out
