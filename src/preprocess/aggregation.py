...
def calculate_day_brand_aggregation(
	df: pd.DataFrame, metric_col: str | None = None
) -> pd.DataFrame:
	"""
	...
	if metric_col and metric_col in df.columns:
		...
		agg = (
			df_metric.groupby(['transaction_date', 'product_brand'], as_index=False)[metric_col]
			.sum()
			.rename(columns={metric_col: f'sum_{metric_col}'})
		)
	else:
		agg = df.groupby(['transaction_date', 'product_brand']).size().reset_index(name='events')
	return agg
