...
def adf_test(series: pd.Series, title: str = '') -> bool:
	s_ = pd.to_numeric(series, errors='coerce').dropna()
	if len(s_) < 10:
		return False
	try:
		result = adfuller(s_, autolag='AIC')
		pvalue = result[1]
		return pvalue < 0.05
	except Exception:
		return False
...
def preparar_datos_prophet(
	y_: pd.Series, exog: Dict[str, Union[pd.Series, np.ndarray]] = None
) -> pd.DataFrame:
	df_p = pd.DataFrame({'ds': y_.index, 'y': y_.values})
	if exog:
		for name, vec in exog.items():
			s_ = pd.Series(vec, index=y_.index)
			df_p[name] = s_.values
	return df_p


def crear_variable_exogena_segura(
	series: pd.Series, periods: List[Tuple[pd.Timestamp, pd.Timestamp]], name: str
) -> pd.Series:
	idx = series.index
...
def create_sequences(
	...
	X_, y_ = [], []
	for i_ in range(seq_len, len(features)):
		X_.append(features[i_ - seq_len : i_])
		y_.append(target[i_])
...
def verificar_no_data_leakage(df: pd.DataFrame, target_col: str, threshold: float = 0.90) -> None:
	...
	for c_ in df.columns:
		if c_ == target_col:
			continue
		eq_rate = (df[c_] == tgt).mean()
		if np.isfinite(eq_rate) and eq_rate >= threshold:
			print(
				f"[warning] Posible fuga de información en '{c_}' (={eq_rate:.2%} iguales a target). Revísalo."
			)
...
class TimeSeriesPreprocessor:
	...
	def load_data(self, file_path: str) -> pd.DataFrame:
		...
		if date_col not in data.columns:
			raise ValueError(
				f"No se encontró columna de fecha (busqué '{date_col}'). Define config.date_col."
			)
		...
		if brand_col in data.columns:
			data[brand_col] = (
				data[brand_col].astype('object').fillna(self.config.brand_unknown_token)
			)
	...
	def _detect_date_col(self, df: pd.DataFrame) -> str:
		...
		candidates = [
			c_
			for c_ in df.columns
			if any(k_ in c_.lower() for k_ in ['date', 'time', 'timestamp', 'datetime'])
		]
		if candidates:
			return candidates[0]
	...
	def _detect_brand_col(self, df: pd.DataFrame) -> str:
		for name in [
			...
			if name in df.columns:
				return name
		for c_ in df.columns:
			if 'brand' in c_.lower():
				return c_
		return '__brand_not_found__'

	def select_brand_series(
		self, df: pd.DataFrame, brand: Optional[str], metric: Optional[str]
	) -> pd.Series:
		brand_col = self.config.brand_col or self._detect_brand_col(df)
		if brand_col not in df.columns:
			raise ValueError(
				"Columna de marca no detectada. Define config.brand_col (p.ej., 'product_brand')."
			)
		...
		y_ = pd.to_numeric(sub[metric_col], errors='coerce')

		# Resample diario
		daily = y_.resample(self.config.freq).sum(min_count=1)
	...
	def _crear_features_completas(self, target_series: pd.Series) -> pd.DataFrame:
		...
		for L_ in self.config.lags:
			df[f'lag_{L_}'] = target_series.shift(L_)
		for w_ in self.config.ventanas_ma:
			df[f'ma_{w_}'] = target_series.shift(1).rolling(w_).mean()
		for w_ in self.config.ventanas_std:
			df[f'std_{w_}'] = target_series.shift(1).rolling(w_).std()
		for w_ in self.config.ventanas_roll:
			df[f'rollsum_{w_}'] = target_series.shift(1).rolling(w_).sum()
		for w_ in self.config.ventanas_momentum:
			df[f'mom_{w_}'] = target_series / target_series.shift(w_) - 1.0
	...
	def create_features(
		self, series: Union[pd.Series, pd.DataFrame], target_col: str = None
	) -> pd.DataFrame:
	...
	def prepare_arima_data(
		self, series: Union[pd.Series, pd.DataFrame], start_date: str = '2000-01-01'
	) -> pd.Series:
	...
	def prepare_prophet_data(
		...
		events_periods: List[Tuple[pd.Timestamp, pd.Timestamp]] = None,
	):
		if isinstance(series, pd.DataFrame):
			num_cols = series.select_dtypes(include=[np.number]).columns
			if len(num_cols) == 0:
		...
		y_ = series.copy()
		if getattr(y_.index, 'tz', None) is not None:
			y_.index = y_.index.tz_convert(None)
		...
		if events_periods:
			...
			exog_vars['event_1'] = crear_variable_exogena_segura(
				y_,
				[(pd.to_datetime(a_), pd.to_datetime(b_)) for (a_, b_) in events_periods],
				'event_1',
			)
		prophet_df = preparar_datos_prophet(y_, exog_vars)
	...
	def prepare_ml_data(
		...
		feature_cols = [c_ for c_ in train_data.columns if c_ not in ['target', 'target_diff']]
	...
	def prepare_lstm_data(
		...
		if feature_columns is None:
			feature_columns = [c_ for c_ in series.columns if c_ != target_column]
		...
		X_, y_ = create_sequences(features_scaled, target_scaled.flatten(), sequence_length)
		split_idx = int(len(X_) * train_ratio)
		X_train, X_test = X_[:split_idx], X_[split_idx:]
		y_train, y_test = y_[:split_idx], y_[split_idx:]
		self.is_fitted = True
	...
	def create_sarimax_exogenous(
		self, series: pd.Series, events_periods: List[Tuple[pd.Timestamp, pd.Timestamp]]
	):
		exog_df = pd.DataFrame(index=series.index)
		for i_, period in enumerate(events_periods):
			var_name = f'event_{i_ + 1}'
			exog_df[var_name] = crear_variable_exogena_segura(series, [period], var_name)
		return exog_df

	# --------- Orquestador cómodo ---------
	def preprocess_for_model(
		self, data_path: str, model_type: str, brand: Optional[str] = None, **kwargs
	):
