...
def synthetic_transaction_data():
	...
	for d_ in dates:
		...
		rows += [{'transaction_date': d_, 'product_brand': 'BrandA', 'product_quantity': 10}] * 3
		# BrandB con 2 transacciones por día
		rows += [{'transaction_date': d_, 'product_brand': 'BrandB', 'product_quantity': 20}] * 2
		# (not set) con 5 transacciones por día
		rows += [{'transaction_date': d_, 'product_brand': '(not set)', 'product_quantity': 5}] * 5
...
def test_calculate_day_brand_aggregation_sum_metric(synthetic_transaction_data):
	...
	result = calculate_day_brand_aggregation(
		synthetic_transaction_data, metric_col='product_quantity'
	)
	...
	data = pd.DataFrame(
		{
			'transaction_date': pd.to_datetime(
				['2016-08-01 14:30:00', '2016-08-01 18:45:00'], utc=True
			),
			'product_brand': ['BrandA', 'BrandA'],
		}
	)
...
def test_generate_day_brand_grouped_data_files_with_metric(
	tmp_aggregation_paths, synthetic_transaction_data
):
