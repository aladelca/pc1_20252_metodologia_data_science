...
# ---------------------
@pytest.fixture()
def tmp_training_paths(tmp_path: Path, request: pytest.FixtureRequest):
...
def synthetic_brand_daily_data():
	...
	for dias in dates:
		...
		n_events = int(100 + 20 * np.sin(dias.dayofyear * 2 * np.pi / 7) + np.random.randn() * 5)
		n_events = max(0, n_events)
		rows.append(
			{
				'transaction_date': dias.strftime('%Y%m%d'),
...
def test_make_features(synthetic_brand_series: pd.Series[float]):
...
def test_make_features_lag_values(synthetic_brand_series: pd.Series[float]):
...
def test_make_features_calendar_variables(synthetic_brand_series: pd.Series[float]):
...
def test_train_xgboost(tmp_training_paths, synthetic_brand_series):
	...
	metrics = train_xgboost(
		series=synthetic_brand_series, test_days=test_days, model_pkl_path=model_path
	)
...
def test_train_xgboost_model_can_predict(tmp_training_paths, synthetic_brand_series):
	...
	# Entrenar
	train_xgboost(series=synthetic_brand_series, test_days=30, model_pkl_path=model_path)
	...
	Xp = feats.drop(columns=['target'])

	# Hacer predicción
	predictions = model.predict(Xp.iloc[-10:])

	# Verificar que devuelve predicciones
	assert len(predictions) == 10
	assert all(isinstance(p_, (int, float, np.number)) for p_ in predictions)
	assert all(p_ >= 0 for p_ in predictions)  # No debe predecir negativos
...
def test_train_xgboost_metrics_quality(
	tmp_training_paths, synthetic_brand_series: pd.Series[float]
):
	"""Test que las métricas están en rangos razonables."""
	model_path = tmp_training_paths['model_file']

	metrics = train_xgboost(series=synthetic_brand_series, test_days=30, model_pkl_path=model_path)
