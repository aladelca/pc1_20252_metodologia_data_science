...
def test_recursive_forecast_xgb_basic(synthetic_series, trained_xgb_model):
	...
	for i_ in range(1, len(dates)):
		delta = (dates.iloc[i_] - dates.iloc[i_ - 1]).days
		assert delta == 1
...
def test_recursive_forecast_xgb_extends_series(synthetic_series, trained_xgb_model):
	...
	forecast = recursive_forecast_xgb(
		series=synthetic_series, model=model, feature_columns=feature_columns, horizon=5
	)
...
def test_recursive_forecast_xgb_uses_predictions(synthetic_series, trained_xgb_model):
	...
	# Pronosticar 2 días
	forecast = recursive_forecast_xgb(
		series=synthetic_series, model=model, feature_columns=feature_columns, horizon=2
	)
...
def test_recursive_forecast_xgb_empty_series():
	...
	forecast = recursive_forecast_xgb(
		series=empty_series, model=model, feature_columns=feature_columns, horizon=3
	)
...
def test_make_features_consistency_with_training():
	...
	for col in train_feats.columns:
		pd.testing.assert_series_equal(train_feats[col], pred_feats[col], check_names=False)
...
def test_recursive_forecast_custom_lags_and_mas(synthetic_series, trained_xgb_model):
	...
	# model = trained_xgb_model["model"]  variable `model` is assigned to but never used

	# Crear features con parámetros personalizados
	custom_feats = make_features(synthetic_series, lags=(1, 2, 3), mas=(3, 5))
	# custom_columns = [c for c in custom_feats.columns if c != "target"] variable `custom_columns` is assigned to but never used

	# Entrenar modelo con estas features
	feats_clean = custom_feats.dropna()
	X_ = feats_clean.drop(columns=['target'])
	y_ = feats_clean['target']

	custom_model = XGBRegressor(n_estimators=30, max_depth=3, random_state=42)
	custom_model.fit(X_, y_)

	# Pronosticar
	forecast = recursive_forecast_xgb(
		series=synthetic_series,
		model=custom_model,
		feature_columns=list(X_.columns),
		horizon=7,
	)
...
def test_forecast_dates_are_timezone_aware(synthetic_series, trained_xgb_model):
	...
	# model = trained_xgb_model["model"]
	# Local variable `feature_columns` is assigned to but never used
	# feature_columns = trained_xgb_model["feature_columns"]

	# local variable `forecast` is assigned to but never used
	# forecast = recursive_forecast_xgb(
	#    series=synthetic_series,
	#    model=model,
	#    feature_columns=feature_columns,
	...
	# dates = pd.to_datetime(forecast["date"])  Local variable `dates` is assigned to but never used
...
def test_recursive_forecast_with_minimal_history(trained_xgb_model):
	...
	forecast = recursive_forecast_xgb(
		series=short_series, model=model, feature_columns=feature_columns, horizon=5
	)
