...
def test_drop_high_missing_columns(synthetic_dirty_data):
	...
	cfg = CleaningConfig(drop_columns_missing_threshold=0.85, drop_columns_missing_action='drop')
	dqr = DataQualityReport()
...
def test_drop_high_missing_columns_impute(synthetic_dirty_data):
	...
	cfg = CleaningConfig(drop_columns_missing_threshold=0.85, drop_columns_missing_action='impute')
	dqr = DataQualityReport()
...
def test_reduce_categoricals_cardinality():
	...
	data = pd.DataFrame({'category': ['A'] * 50 + ['B'] * 30 + ['C'] * 5 + ['D'] * 5 + ['E'] * 3})
