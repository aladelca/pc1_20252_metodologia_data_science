...
def synthetic_raw_df():
	...
	for d_ in dates:
		...
		rows += [{'transaction_date': d_.strftime('%Y%m%d'), 'product_brand': '(not set)'}] * k1
		# otra marca con multiplicidad 0..3
		k2 = int(rng.integers(0, 4))
		rows += [{'transaction_date': d_.strftime('%Y%m%d'), 'product_brand': 'BrandX'}] * k2
...
def test_end_to_end_agg_and_preprocess(tmp_paths, synthetic_raw_df):
	...
	assert (
		...
		.equals(agg_disk[cols].sort_values(cols).reset_index(drop=True))
	)
